...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        "*** YOUR CODE HERE ***"
        return self.place.ant is not None and self.place.ant.blocks_path #checks if ant is there and if it blocks path
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while current_place != None and current_place != hive and counter <= self.max_range:
            if random_or_none(current_place.bees):
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        self.armor -= amount
        if self.armor <= 0:
            for bees in self.place.bees[:]: #slices the array and makes copy
...
class ScubaThrower(ThrowerAnt):
    "A ThrowerAnt that is more costly and is watersafe"
    name = "Scuba"
    watersafe = True
    food_cost = 5
    armor = 1
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self):
        "*** YOUR CODE HERE ***"
        Ant.__init__(self, 1)
        QueenAnt.number_Queen += 1
        if QueenAnt.number_Queen > 1:
            self.is_imposter = True

    def double_my_damage_by_two_divided_by_two(self, ant):
        if ant is not None:
            if ant not in self.ants_doubled:
                if type(ant) is not QueenAnt:
    ...
    def action(self, colony):
        ...
        if self.is_imposter:
            ...
            while current_place is not None:
                self.double_my_damage_by_two_divided_by_two(current_place.ant)
                if type(current_place.ant) is BodyguardAnt and type(current_place.ant.ant) is not QueenAnt:
                    self.double_my_damage_by_two_divided_by_two(current_place.ant.ant)
                current_place = current_place.entrance
...
def apply_effect(effect, bee, duration):
    ...
    "*** YOUR CODE HERE ***"
    length_of_time = 0
    regular_action = bee.action
    def change_action(colony):
        nonlocal length_of_time
        nonlocal regular_action
        length_of_time+=1
        if length_of_time <= duration:
            return effect(regular_action)(colony)
        else:
            return regular_action(colony)
